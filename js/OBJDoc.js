...
OBJDoc.prototype.parse = function(fileString, scale, reverse) {
  ...
  while ((line = lines[index++]) != null) {
    ...
    case 'vn':   // Read normal
      ...
      continue; // Go to the next line
    case 'vt':   // Read normal
      var texture = this.parseNormal(sp);
...
OBJDoc.prototype.parseFace = function(sp, materialName, vertices, reverse) {  
  ...
  for(;;){
    ...
    if(subWords.length >= 3){
      ...
      face.vTexture.push( parseInt(subWords[1]) - 1 );
    }else{
  ...
  if(face.vIndices.length > 3){
    ...
    for(var i=0; i<n; i++){
      ...
      newVTIndices[i * 3 + 0] = face.vTexture[0];
      newVTIndices[i * 3 + 1] = face.vTexture[i + 1];
      newVTIndices[i * 3 + 2] = face.vTexture[i + 2];      
    }
    face.vIndices = newVIndices;
    face.nIndices = newNIndices;
    face.vTexture = newVTIndices;
  }
...
OBJDoc.prototype.getDrawingInfo = function() {
  ...
  var textures = new Float32Array(numVertices * 3);
    ...
    for(var j = 0; j < object.faces.length; j++){
      ...
      for(var k = 0; k < face.vIndices.length; k++){
        ...
        if(nIdx >= 0){
          ...
          normal = this.textures[k];
          textures[index_indices * 3 + 0] = normal.x;
          textures[index_indices * 3 + 1] = normal.y;
          textures[index_indices * 3 + 2] = normal.z;
...
StringParser.prototype.skipDelimiters = function()  {
  for(var i = this.index, len = this.str.length; i < len; i++){
    ...
    // Skip TAB, Space, '(', ')
    if (c == '\t'|| c == ' ' || c == '(' || c == ')' || c == '"' || c.charCodeAt() == 13) continue;
    break;
  }
  this.index = i;
}
...
Core.createOBJDoc = function( fileName ) {
