...
function TextureShader( gl , images) {
    ...
    this.textureType = images.length == 6 ? 'box' : 'face';
    this.shader = new Core.Shader( gl, [
        'attribute vec3 a_Position;',
        'uniform mat4 u_MvpMatrix;',
        this.textureType == 'box' ? 'varying vec3 v_Color;' : ('varying vec2 v_TexCoord;\n' + 'attribute vec2 a_TexCoord;'),
        'void main( void ){',
        ...
        'void main( void ){',
        this.textureType == 'box' ? '  gl_FragColor = textureCube(u_Sampler,v_Color);' : ' gl_FragColor = texture2D(u_Sampler,v_TexCoord);',
        '}'
    ].join('\n'));
    this.index = {};
    this.buffer = {};
